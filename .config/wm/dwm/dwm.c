...
#define SYSTEM_TRAY_ICON_PADDING    3
/* XEMBED messages */
...
drawstatusbar(Monitor *m, int bh, char* stext) {
    ...
    /* NOTE: adjust systray layout to avoid collision with status2d on left-side placement */
    /* https://www.reddit.com/r/suckless/comments/rcn9vg/dwm_status2d_and_systray_overlapping */
    ret = m->ww - w;
...
{
    int nbh = bh - SYSTEM_TRAY_ICON_PADDING;
    ...
    if (i) {
        ...
        if (w == h)
            ...
            i->w = (int)((float)nbh * ((float)w / (float)h));
		...
		if (i->h > nbh) {
			if (i->w == i->h)
				...
				i->w = (int) ((float)nbh * ((float)i->w / (float)i->h));
			i->h = nbh;
		}
    }
}
