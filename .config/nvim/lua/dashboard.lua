...
local config = {
  ...
  shortcuts = {
    ...
    { key = 'h', desc = 'Help Pages', action = '<cmd>FzfLua helptags<cr>' },
...
local function render_dashboard(buf)
  ...
  local total_lines =
    math.max(config.layout.top_offset + lambda_lines, shortcuts_start_idx + #config.shortcuts, plugin_info_line_idx + 1)
  ...
  --   string.format('load %d/%d plugins in %sms', vim.g.strive_loaded or 0, vim.g.strive_count or 0, startup_time)
  --
